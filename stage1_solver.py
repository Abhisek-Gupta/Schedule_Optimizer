import pulp
import mysql.connector
import pandas as pd

# ==========================================
# 1. CONFIGURATION & TIME DOMAINS
# ==========================================
DB_CONFIG = {
    'host': 'localhost',
    'user': 'root',
    'password': 'password',
    'database': 'IITP_Timetable'
}

DAYS = 5
SLOTS_PER_DAY = 8
TOTAL_SLOTS = DAYS * SLOTS_PER_DAY

VALID_LAB_STARTS_PER_DAY = [0, 1, 4, 5] 
ALL_VALID_LAB_STARTS = [d * SLOTS_PER_DAY + s for d in range(DAYS) for s in VALID_LAB_STARTS_PER_DAY]
ALL_VALID_LEC_STARTS = list(range(TOTAL_SLOTS))
GROUPS = list(range(1, 25))

W_GAP = 5     
W_OVERLOAD = 10 

# ==========================================
# 2. EVENT DEFINITION
# ==========================================
class Event:
    def __init__(self, e_id, course, e_type, groups, duration, valid_rooms, valid_times, faculty):
        self.id = e_id
        self.course = course
        self.type = e_type
        self.groups = groups
        self.duration = duration
        self.valid_rooms = valid_rooms
        self.valid_times = valid_times
        self.faculty = faculty 

# ==========================================
# 3. DB EXTRACTION & EVENT SHATTERING
# ==========================================
def fetch_context(conn):
    rooms_df = pd.read_sql("SELECT Room_name, Capacity, Type, Department FROM classrooms_labs", conn)
    fac_df = pd.read_sql("SELECT Course_ID, Fac_ID FROM course_instructor", conn)
    
    lec_rooms = rooms_df[(rooms_df['Type'] == 'CLASSROOM') & (rooms_df['Capacity'] >= 270)]['Room_name'].tolist()
    
    lab_dict = {
        'CS1201': rooms_df[(rooms_df['Room_name'] == 'CC1')]['Room_name'].tolist(),
        'CH1201': rooms_df[(rooms_df['Room_name'] == 'CHLAB1')]['Room_name'].tolist(),
        'PH1201': rooms_df[(rooms_df['Room_name'] == 'PHLAB1')]['Room_name'].tolist(),
        'ME1201': rooms_df[(rooms_df['Room_name'] == 'MEWORKSHOP')]['Room_name'].tolist(),
        'EE1201': rooms_df[(rooms_df['Room_name'] == 'EELAB1')]['Room_name'].tolist(),
    }
    
    instructors = {}
    for course in ['MA1201', 'CS1201', 'CH1201', 'ME1201', 'ME1202', 'PH1201', 'EE1201', 'CE1201']:
        facs = fac_df[fac_df['Course_ID'] == course]['Fac_ID'].tolist()
        instructors[course] = facs if len(facs) >= 2 else [f"{course}_F1", f"{course}_F2"]
        
    return lec_rooms, lab_dict, instructors

def generate_events(lec_rooms, lab_dict, instructors):
    events = []
    e_counter = 0
    
    # --- CORRECTED FACULTY ASSIGNMENT LOGIC ---
    def assign_fac(course, g_range):
        first_group = g_range[0]
        
        # Common Courses (24 Groups): Split at Group 12
        if course in ['MA1201', 'CS1201']:
            return instructors[course][0] if first_group <= 12 else instructors[course][1]
            
        # G1-12 Track (12 Groups): Split at Group 6
        elif course in ['CH1201', 'ME1201', 'ME1202']:
            return instructors[course][0] if first_group <= 6 else instructors[course][1]
            
        # G13-24 Track (12 Groups): Split at Group 18
        elif course in ['PH1201', 'EE1201', 'CE1201']:
            return instructors[course][0] if first_group <= 18 else instructors[course][1]

    # 1. Common Courses
    for course, l_hrs in [('MA1201', 3), ('CS1201', 3)]:
        for _ in range(l_hrs):
            for g_range in [range(1,7), range(7,13), range(13,19), range(19,25)]:
                fac = assign_fac(course, g_range)
                events.append(Event(f"E{e_counter}", course, 'LEC', list(g_range), 1, lec_rooms, ALL_VALID_LEC_STARTS, fac))
                e_counter += 1
                
    for g_range in [range(i, i+3) for i in range(1, 25, 3)]:
        fac = assign_fac('CS1201', g_range)
        events.append(Event(f"E{e_counter}", 'CS1201', 'LAB', list(g_range), 3, lab_dict['CS1201'], ALL_VALID_LAB_STARTS, fac))
        e_counter += 1

    # 2. G1-12 Track 
    for course in ['CH1201', 'ME1202']:
        for _ in range(3):
            for g_range in [range(1,7), range(7,13)]:
                fac = assign_fac(course, g_range)
                events.append(Event(f"E{e_counter}", course, 'LEC', list(g_range), 1, lec_rooms, ALL_VALID_LEC_STARTS, fac))
                e_counter += 1
                
    for course in ['CH1201', 'ME1201']:
        for g_range in [range(i, i+3) for i in range(1, 13, 3)]:
            fac = assign_fac(course, g_range)
            events.append(Event(f"E{e_counter}", course, 'LAB', list(g_range), 3, lab_dict[course], ALL_VALID_LAB_STARTS, fac))
            e_counter += 1

    # 3. G13-24 Track 
    for course, l_hrs in [('PH1201', 3), ('EE1201', 3), ('CE1201', 1)]:
        for _ in range(l_hrs):
            for g_range in [range(13,19), range(19,25)]:
                fac = assign_fac(course, g_range)
                events.append(Event(f"E{e_counter}", course, 'LEC', list(g_range), 1, lec_rooms, ALL_VALID_LEC_STARTS, fac))
                e_counter += 1
                
    for course in ['PH1201', 'EE1201']:
        for g_range in [range(i, i+3) for i in range(13, 25, 3)]:
            fac = assign_fac(course, g_range)
            events.append(Event(f"E{e_counter}", course, 'LAB', list(g_range), 3, lab_dict[course], ALL_VALID_LAB_STARTS, fac))
            e_counter += 1

    for g_range in [range(13,19), range(19,25)]:
        fac = assign_fac('CE1201', g_range)
        events.append(Event(f"E{e_counter}", 'CE1201', 'LAB', list(g_range), 3, lec_rooms, ALL_VALID_LAB_STARTS, fac))
        e_counter += 1

    return events

# ==========================================
# 4. ILP FORMULATION & SOLVER
# ==========================================
def solve_timetable(events):
    print(f"Formulating ILP Model for {len(events)} events...")
    prob = pulp.LpProblem("IITP_Stage1_Optimization", pulp.LpMinimize)

    x = {e.id: {r: {t: pulp.LpVariable(f"x_{e.id}_{r}_{t}", cat=pulp.LpBinary) 
                    for t in e.valid_times} for r in e.valid_rooms} for e in events}

    all_faculties = set(e.faculty for e in events)

    # --- HARD CONSTRAINTS ---
    for e in events:
        prob += pulp.lpSum([x[e.id][r][t] for r in e.valid_rooms for t in e.valid_times]) == 1

    all_rooms = set(r for e in events for r in e.valid_rooms)
    for t in range(TOTAL_SLOTS):
        day = t // SLOTS_PER_DAY
        
        # Room Capacity
        for r in all_rooms:
            active_in_r = [x[e.id][r][start_t] for e in events if r in e.valid_rooms 
                           for start_t in range(max(0, t - e.duration + 1), t + 1) 
                           if start_t in e.valid_times and start_t // SLOTS_PER_DAY == day]
            if active_in_r:
                prob += pulp.lpSum(active_in_r) <= 1

        # Group Capacity
        for g in GROUPS:
            active_for_g = [x[e.id][r][start_t] for e in events if g in e.groups for r in e.valid_rooms 
                            for start_t in range(max(0, t - e.duration + 1), t + 1) 
                            if start_t in e.valid_times and start_t // SLOTS_PER_DAY == day]
            if active_for_g:
                prob += pulp.lpSum(active_for_g) <= 1
                
        # Faculty Capacity
        for f in all_faculties:
            active_for_f = [x[e.id][r][start_t] for e in events if e.faculty == f for r in e.valid_rooms
                            for start_t in range(max(0, t - e.duration + 1), t + 1) 
                            if start_t in e.valid_times and start_t // SLOTS_PER_DAY == day]
            if active_for_f:
                prob += pulp.lpSum(active_for_f) <= 1

    # Max 1 Lecture per course per day
    for d in range(DAYS):
        day_slots = range(d * SLOTS_PER_DAY, (d + 1) * SLOTS_PER_DAY)
        for g in GROUPS:
            for course in set(e.course for e in events):
                lecs = [e for e in events if e.course == course and e.type == 'LEC' and g in e.groups]
                starts_today = [x[e.id][r][t] for e in lecs for r in e.valid_rooms for t in e.valid_times if t in day_slots]
                if starts_today:
                    prob += pulp.lpSum(starts_today) <= 1

    # Symmetry Breaking
    ph_labs = [e for e in events if e.course == 'PH1201' and e.type == 'LAB']
    ch_labs = [e for e in events if e.course == 'CH1201' and e.type == 'LAB']
    for ph_e, ch_e in zip(ph_labs, ch_labs):
        for t in ph_e.valid_times:
            prob += pulp.lpSum([x[ph_e.id][r][t] for r in ph_e.valid_rooms]) == pulp.lpSum([x[ch_e.id][r][t] for r in ch_e.valid_rooms])

    ee_labs = [e for e in events if e.course == 'EE1201' and e.type == 'LAB']
    me_labs = [e for e in events if e.course == 'ME1201' and e.type == 'LAB']
    for ee_e, me_e in zip(ee_labs, me_labs):
        for t in ee_e.valid_times:
            prob += pulp.lpSum([x[ee_e.id][r][t] for r in ee_e.valid_rooms]) == pulp.lpSum([x[me_e.id][r][t] for r in me_e.valid_rooms])

    # --- SOFT CONSTRAINTS ---
    fac_overload = {}
    for f in all_faculties:
        for d in range(DAYS):
            day_slots = range(d * SLOTS_PER_DAY, (d + 1) * SLOTS_PER_DAY)
            daily_hours = pulp.lpSum([e.duration * x[e.id][r][t] for e in events if e.faculty == f 
                                      for r in e.valid_rooms for t in e.valid_times if t in day_slots])
            
            prob += daily_hours <= 8, f"Fac_MaxHours_{f}_Day{d}"
            
            overload_var = pulp.LpVariable(f"overload_{f}_{d}", lowBound=0, cat=pulp.LpContinuous)
            prob += overload_var >= daily_hours - 4
            fac_overload[(f, d)] = overload_var

    group_span = {}
    for g in GROUPS:
        for d in range(DAYS):
            day_slots = range(d * SLOTS_PER_DAY, (d + 1) * SLOTS_PER_DAY)
            max_t = pulp.LpVariable(f"max_t_{g}_{d}", lowBound=0, cat=pulp.LpInteger)
            min_t = pulp.LpVariable(f"min_t_{g}_{d}", lowBound=0, upBound=SLOTS_PER_DAY, cat=pulp.LpInteger)
            has_class = pulp.LpVariable(f"active_{g}_{d}", cat=pulp.LpBinary)
            
            for e in [ev for ev in events if g in ev.groups]:
                for r in e.valid_rooms:
                    for t in e.valid_times:
                        if t in day_slots:
                            slot_idx = t % SLOTS_PER_DAY
                            prob += max_t >= (slot_idx + e.duration) * x[e.id][r][t]
                            prob += min_t <= slot_idx * x[e.id][r][t] + 10 * (1 - x[e.id][r][t])
                            
            total_classes = pulp.lpSum([x[e.id][r][t] for e in events if g in e.groups 
                                        for r in e.valid_rooms for t in e.valid_times if t in day_slots])
            prob += total_classes <= 20 * has_class
            prob += min_t <= 10 * (1 - has_class)
            
            span_var = pulp.LpVariable(f"span_{g}_{d}", lowBound=0, cat=pulp.LpInteger)
            prob += span_var >= max_t - min_t
            group_span[(g, d)] = span_var

    # --- OBJECTIVE FUNCTION ---
    prob += pulp.lpSum([W_GAP * group_span[(g, d)] for g in GROUPS for d in range(DAYS)]) + \
            pulp.lpSum([W_OVERLOAD * fac_overload[(f, d)] for f in all_faculties for d in range(DAYS)])

    print("Solving Phase 1 Objective...")
    # Turn msg=True to see the logs. 
    # Set timeLimit to 60 seconds.
    # gapRel=0.05 tells it to stop if it is within 5% of the mathematically perfect optimal score.
    prob.solve(pulp.PULP_CBC_CMD(msg=True, timeLimit=600, gapRel=0.05))
    # print(f"Solver Status: {pulp.LpStatus[prob.status]}")
    
    schedule = []
    # Check if it found the perfect mathematical solution OR stopped early with a valid timetable
    if prob.status == pulp.LpStatusOptimal or prob.status == pulp.LpStatusNotSolved:
        for e in events:
            for r in e.valid_rooms:
                for t in e.valid_times:
                    if pulp.value(x[e.id][r][t]) == 1.0:
                        schedule.append((e.id, e.course, e.type, f"G{e.groups[0]}-{e.groups[-1]}", r, t, e.duration, e.faculty))
    return schedule

# ==========================================
# 5. DB INSERTION
# ==========================================
def save_to_database(conn, schedule):
    cursor = conn.cursor()
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS stage1_timetable (
            ID INT AUTO_INCREMENT PRIMARY KEY,
            Event_ID VARCHAR(20),
            Course_ID VARCHAR(10),
            Type VARCHAR(10),
            Groups VARCHAR(20),
            Room_name VARCHAR(20),
            Start_Slot INT,
            Duration INT,
            Fac_ID VARCHAR(10)
        )
    """)
    cursor.execute("TRUNCATE TABLE stage1_timetable")
    insert_query = """
        INSERT INTO stage1_timetable (Event_ID, Course_ID, Type, Groups, Room_name, Start_Slot, Duration, Fac_ID)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
    """
    cursor.executemany(insert_query, schedule)
    conn.commit()
    print(f"Saved {len(schedule)} optimized rows to MySQL.")

if __name__ == "__main__":
    connection = mysql.connector.connect(**DB_CONFIG)
    lec_rooms, lab_dict, instructors = fetch_context(connection)
    events = generate_events(lec_rooms, lab_dict, instructors)
    final_schedule = solve_timetable(events)
    if final_schedule:
        save_to_database(connection, final_schedule)
    connection.close()